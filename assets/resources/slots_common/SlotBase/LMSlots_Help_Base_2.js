...
cc.Class({
    ...
    onLoad () {
        ...
        cc.find("footeritem", this.node).active = false;
    ...
    InitPages:function(){
        ...
        if(cfg && cfg.helpItems){
            for(let i = 0; i < cfg.helpItems.length; i++) {
                let item = cc.instantiate(cc.find("footeritem", this.node));
                item.parent = cc.find("footer", this.node);
                item.active = true;
                this._footerList.push(item);
            }
        }
        this.showFooter(0);
    },
    ...
    showFooter(idx){
        this._currPage = idx;
        for (let i = 0; i < this._footerList.length; i++){
            cc.find("light",this._footerList[i]).active = i == idx;
        }
    },
    ...
    ShowUIAction:function(){
        ...
        .to(0.5,{y:0})
        .call(() => {
            let btn_back = cc.find("popup_close",self.node)
