...
cc.Class({
    ...
    onLoad () {
        for (let i = 1; i <= 6; i++) {
            let item = cc.find("bg/layout/item"+i, this.node);
    ...
    start () {
        cc.vv.NetManager.sendAndCache({ c: MsgId.REQ_SHOP_EX, stype: "31", platform: Global.isIOS() ? 2 : 1 }, true);
    },
    ...
    REQ_SHOP_EX(msg) {
        ...
        this.localData = [];
        if (shoplist.data31) {
            this.localData = shoplist.data31;
            this.updateView();
        }
    },
    ...
    updateView() {
        for (let i = 0; i < this.itemList.length; i++) {
            ...
            if (data) {
                ...
                cc.find("btn/label", itemNode).getComponent(cc.Label).string = "₹" + Global.FormatNumToComma(data.amount);
    ...
    onClickPay(itemNode) {
        ...
        if (data) {
            StatisticsMgr.reqReportNow(ReportConfig.new_quickpaydlg_click, data.id);
