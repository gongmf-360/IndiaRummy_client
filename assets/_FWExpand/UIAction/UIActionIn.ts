...
@ccclass
@menu("UI动作/进入")
export default class UIActionIn extends cc.Component {
    @property
    auto = true;
    @property
    animTime: number = 0.3;
    ...
    run() {
        ...
        if (this.actionType == ActionType.SCALE) {
            this.node.scale = 0.2;
            tween = cc.tween().to(this.animTime, { scale: 1 }, { easing: easingName })
        } else if (this.actionType == ActionType.MOVE) {
            this.node.position = this.initPos.add(this.offsetPos);
            tween = cc.tween().to(this.animTime, { position: this.initPos }, { easing: easingName })
        }
        cc.tween(this.node).then(tween).call(() => {
            for (const func of this.endCallFuncs) {
                func(this.node);
            }
        }).start();
    }
