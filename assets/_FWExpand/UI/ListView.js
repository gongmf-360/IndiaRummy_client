...
const ListItem = require('ListViewItem');
...
cc.Class({
        ...
        if (this._virtual) {
            ...
            if (this._forceUpdate || haveDataChange) {    //如果是强制更新
                if (this.frameByFrameRenderNum > 0) {
                    ...
                    //直接渲染
                    this._lastDisplayData = [];
                    // cc.log('List Display Data II::', this.displayItemNum, this.displayData);
    ...
    _createOrUpdateItem(data) {
        ...
        if (!item) { //如果不存在
            let isCreate = false;
            let canGet = this._pool.length > 0;
            if (canGet) {
                // cc.log('_createOrUpdateItem', data.id);
                item = this._pool.pop();
            } else {
                isCreate = true;
            ...
            // cc.log("item._listId 赋值:", item._listId, data.id)
    ...
    getItemByListId(listId) {
        if (this.content) {
            for (let n = this.content.childrenCount - 1; n >= 0; n--) {
                if (this.content.children[n].active && this.content.children[n]._listId == listId)
    ...
    _getOutsideItem() {
        ...
        for (let n = this.content.childrenCount - 1; n >= 0; n--) {
            ...
            if (item.active && !this.displayData.find(d => d.id == item._listId)) {
                result.push(item);
            }
        }
        return result;
    ...
    _delRedundantItem() {
        if (this._virtual) {
            ...
            for (let n = arr.length - 1; n >= 0; n--) {
                ...
                item.isCached = true;
                item.active = false;
                // cc.log('存入:: pool.length =', this._pool.length, "listId:", item._listId);
