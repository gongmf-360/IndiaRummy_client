...
@ccclass
export default class CarouselCpt extends cc.Component {
    ...
    updateView() {
        ...
        for (let i = 0; i < this.showItemList.length; i++) {
            ...
            if (i == 0) {
                ...
                itemCpt.node.x = this.node.width;
            }
        }
        this.curIdx = 0;
    }
    ...
    onToNext() {
        ...
        cc.tween(this.indexItem.node).to(0.4, { x: -this.node.width }).start();
        cc.tween(this.nextItem.node)
            .to(0.4, { x: 0 })
            .call(() => {
