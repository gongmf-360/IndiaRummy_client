...
export default class EcsUtils {
	...
	public static formatBigNum(num: number, fmtLen: number = 5, digits: number = 2): string {
		...
		if (numStr.length < fmtLen) {
			return EcsUtils.formatNumWithSymbol(num);
		}
		let cfg = null;
	...
	public static deepClone = function (obj) {
		...
		if (isArr) {
			...
			for (let i = 0; i < obj.length; i++) {
				newObj[i] = EcsUtils.deepClone(obj[i]);
			}
			return newObj;
		} else if (isJson) {
			// 克隆Object
	...
	public static randomColor(args?: any) {
		return cc.color(EcsUtils.randomBy(0, 255), EcsUtils.randomBy(0, 255), EcsUtils.randomBy(0, 255));
	}

	//随机牌值
	public static randomCardUint(args?: any) {
		return 0x100 * EcsUtils.randomBy(1, 4) + EcsUtils.randomBy(2, 14);
	}
