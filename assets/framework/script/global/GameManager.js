...
cc.Class({
    extends: cc.Component, statics: {
        _interval_id: null, _showExit: false,        // 防止多次弹出退出框
        _isBackground: false,
        ...
        reqReLogin: function (bAuto) {
            ...
            if (preLoginStr) {
                console.log("协议1StatisticsMgr：：：", preLoginStr);
        ...
        constructLoginMsg: function (nickname, pwd, loginType, accesstoken, loginExData, token) {
            ...
            req.t = loginType; //1随机 2 微信 3fb 7.token登录
            ...
            if (msgDic.code === 200) {
                ...
                if (msgDic.deskFlag == 1) {
                    ...
                    if (loginType == Global.LoginType.APILOGIN) {
                        //api调用 登陆成功后 直接进入游戏
                        let nId = this.getUrlParams("gameid");
