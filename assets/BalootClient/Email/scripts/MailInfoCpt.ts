...
export default class MailInfoCpt extends cc.Component {
    ...
    updateView(data) {
        ...
        cc.find('lbl_time', infoNode).getComponent(cc.Label).string = Global.getTimeStr(data.sendtime);
        cc.find('lbl_content', infoNode).getComponent(cc.Label).string = data.msg;
        cc.find('sender/lbl_sender', infoNode).getComponent(cc.Label).string = data.sender;
